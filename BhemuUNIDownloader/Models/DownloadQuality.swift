...
enum DownloadQuality: String, CaseIterable, Identifiable {
    ...
    var formatArguments: [String] {
        ...
        case .p1080:
            ...
            return ["-f", "bv*[vcodec^=avc1][height<=1080]+ba/bv*[ext=mp4][height<=1080]+ba/bestvideo[height<=1080]+bestaudio/best[height<=1080]", "--merge-output-format", "mp4"]
        case .p720:
            // 720p H.264 MP4
            return ["-f", "bv*[vcodec^=avc1][height<=720]+ba/bv*[ext=mp4][height<=720]+ba/bestvideo[height<=720]+bestaudio/best[height<=720]", "--merge-output-format", "mp4"]
        case .p480:
            // 480p H.264 MP4
            return ["-f", "bv*[vcodec^=avc1][height<=480]+ba/bv*[ext=mp4][height<=480]+ba/bestvideo[height<=480]+bestaudio/best[height<=480]", "--merge-output-format", "mp4"]
        case .audioOnly:
